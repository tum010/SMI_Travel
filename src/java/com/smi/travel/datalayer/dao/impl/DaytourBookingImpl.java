...
public class DaytourBookingImpl implements DaytourBookingDao {
    ...
    private List getDailyTourList(String from, String to, String department) {
        ...
        String query = " SELECT db.tour_id AS tourid, dt.`code` AS tourcode, dt.`name` AS tourname, db.tour_date AS tourdate, sum(db.adult) AS ad, sum(db.child) AS ch, sum(db.infant) AS Inf, sum( GET_PRICE_DAYTOUR_BOOK (db.id)) AS sell, ( ifnull( GET_COST_DAYTOUR_FROM_TOURDATE (db.tour_id, db.tour_date), 0 )) AS net, sum( GET_PRICE_DAYTOUR_BOOK (db.id)) - ( ifnull( GET_COST_DAYTOUR_FROM_TOURDATE (db.tour_id, db.tour_date), 0 )) AS balance, CASE WHEN ifnull(sum(db.adult), 0) + ifnull(sum(db.child), 0) + ifnull(sum(db.infant), 0) = 0 THEN 0 ELSE round(( sum( GET_PRICE_DAYTOUR_BOOK (db.id)) - ( ifnull( GET_COST_DAYTOUR_FROM_TOURDATE (db.tour_id, db.tour_date), 0 ))) / ( ifnull(sum(db.adult), 0) + ifnull(sum(db.child), 0) + ifnull(sum(db.infant), 0)), 2 ) END AS average FROM `daytour_booking` db INNER JOIN daytour dt ON dt.id = db.tour_id JOIN `master` `mt` ON (`mt`.`id` = `db`.`master_id`) ";
