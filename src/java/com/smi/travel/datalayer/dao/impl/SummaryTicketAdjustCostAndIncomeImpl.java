...
public class SummaryTicketAdjustCostAndIncomeImpl implements SummaryTicketAdjustCostAndIncomeDao{
    ...
    public List<ListSummaryTicketAdjustCostAndIncome> getSummaryTicketAdjustCostAndIncome(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            query = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - `fare`.`sale_price` )) AS `costinv`, ifnull( sum( `GET_AMOUNT_TICKET_FARE` (`fare`.`id`, 'wendy')), 0 ) AS `invoicewendy`, ifnull( sum( `GET_AMOUNT_TICKET_FARE` (`fare`.`id`, 'inbound')), 0 ) AS `invoiceinbound`, ifnull( sum( `GET_AMOUNT_TICKET_FARE` (`fare`.`id`, 'outbound')), 0 ) AS `invoiceoutbound`, sum( ifnull( `fare`.`agent_commission`, 0 )) AS `discount`, `GET_CANCEL_AMOUNT_TICKET_FARE` (`fare`.`id`) AS `cancel`, ( SELECT sum(`air`.`inv_amount`) FROM `ticket_fare_airline` `air` WHERE ((`air`.`id` = `fare`.`id`) AND (`air`.`is_wait_pay` = 1))) AS `wait_pay`, sum( ifnull(`fare`.`diff_vat`, 0)) AS `rcagcom`, sum( ifnull(`fare`.`over_commission`, 0)) AS `over`, sum( ifnull( `fare`.`litter_commission`, 0 )) AS `litter`, sum((((((( ifnull(`fare`.`inv_amount`, 0) + ifnull(`fare`.`over_commission`, 0)) - ifnull( `fare`.`litter_commission`, 0 )) + ifnull( `fare`.`agent_commission`, 0 )) - ifnull(`fare`.`diff_vat`, 0)) - ifnull(( SELECT sum(`air`.`inv_amount`) FROM `ticket_fare_airline` `air` WHERE ((`air`.`id` = `fare`.`id`) AND (`air`.`is_wait_pay` = 1))), 0 )) - ifnull( `GET_CANCEL_AMOUNT_TICKET_FARE` (`fare`.`id`), 0 ))) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`))) " ; 
    ...
    public List getSummaryTicketCostAndIncome(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        System.out.println("Term Pay : " + termPayt);
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            query = "select (case when (`fare`.`ticket_type` = 'B') then 'BSP' when (`fare`.`ticket_type` = 'A') then 'AGENT' when (`fare`.`ticket_type` = 'D') then 'DOMESTIC' when (`fare`.`ticket_type` = 'TI') then 'TG INTER' when (`fare`.`ticket_type` = 'TD') then 'TG DOMESTIC' else `fare`.`ticket_type` end) AS `typepayment`,`fare`.`ticket_rounting` AS `typerounting`,count(`fare`.`ticket_no`) AS `pax`,substr(`fare`.`ticket_no`,1,3) AS `air`,`GET_TICKET_ISSUE`(`fare`.`ticket_type`,`fare`.`ticket_rounting`,substr(`fare`.`ticket_no`,1,3)) AS `ticketissue`,(case when (`inv`.`inv_type` = 'T') then substr(`inv`.`inv_no`,1,5) else substr(`inv`.`inv_no`,1,6) end) AS `invno`,sum(`GET_COST_TICKETFARE`(`fare`.`id`)) AS `cost`,sum(`GET_SALE_PRICE_TICKET_FARE`(`fare`.`id`,'Inbound')) AS `inbound`,sum(`GET_SALE_PRICE_TICKET_FARE`(`fare`.`id`,'Wendy')) AS `wendy`,sum(`GET_SALE_PRICE_TICKET_FARE`(`fare`.`id`,'Outbound')) AS `outbound`,(case when (`fare`.`pv_type` = 9) then sum(`fare`.`sale_price`) else NULL end) AS `refund`,(case when (`fare`.`pv_type` = 7) then sum(`fare`.`sale_price`) else NULL end) AS `businesstrip`,(case when (`fare`.`pv_type` = 8) then sum(`fare`.`sale_price`) else NULL end) AS `annualleave`,(case when (`fare`.`pv_type` = 10) then sum(`fare`.`sale_price`) else NULL end) AS `noinvoice`,sum((`GET_COST_TICKETFARE`(`fare`.`id`) - `fare`.`sale_price`)) AS `costinv`,(case when ((`fare`.`department` = 'Wendy') and isnull(`fare`.`pv_type`)) then sum(`fare`.`inv_amount`) else NULL end) AS `invwendy`,(case when ((`fare`.`department` = 'Outbound') and isnull(`fare`.`pv_type`)) then sum(`fare`.`inv_amount`) else NULL end) AS `invoutbound`,(case when (`fare`.`pv_type` = 9) then sum(`fare`.`inv_amount`) else NULL end) AS `invrefund`,(case when (`fare`.`pv_type` = 7) then sum(`fare`.`inv_amount`) else NULL end) AS `invbusinesstrip`,(case when (`fare`.`pv_type` = 8) then sum(`fare`.`inv_amount`) else NULL end) AS `invannualleave`,(case when (`fare`.`pv_type` = 10) then sum(`fare`.`inv_amount`) else NULL end) AS `invnoinvoice` from (((`ticket_fare_airline` `fare` left join `ticket_fare_invoice` `finv` on((`finv`.`ticket_fare_id` = `fare`.`id`))) left join `invoice` `inv` on((`inv`.`id` = `finv`.`invoice_id`))) left join `m_accterm` `term` on((`term`.`id` = `inv`.`term_pay`)))  " ; 
