...
public class SummaryTicketAdjustCostAndIncomeImpl implements SummaryTicketAdjustCostAndIncomeDao{
    ...
    public List<ListSummaryTicketAdjustCostAndIncome> getSummaryTicketAdjustCostAndIncome(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            query = " SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, substr(fare.ticket_no, 1, 3) AS air, count(`fare`.`ticket_no`) AS `pax`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) WHEN fare.pv_code IS NOT NULL THEN substr(fare.pv_code, 1, 6) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' AND inv.inv_no IS NOT NULL ) THEN `fare`.`inv_amount` ELSE 0 END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' AND inv.inv_no IS NOT NULL ) THEN `fare`.`inv_amount` ELSE 0 END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' AND inv.inv_no IS NOT NULL ) THEN `fare`.`inv_amount` ELSE 0 END )), 0 ) AS `invoiceoutbound`, sum( CASE WHEN ( fare.diff_vat < 0 OR fare.pv_type IS NOT NULL ) AND fare.ticket_type = 'TD' THEN ifnull(`fare`.agent_commission, 0) ELSE 0 END ) AS `discount`, sum( CASE WHEN fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.inv_amount, 0) ELSE 0 END ) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.inv_amount, 0) ELSE 0 END )) AS `wait_pay`, sum( CASE WHEN fare.ticket_type = 'A' AND `fare`.`is_wait_pay` = 0 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `rcagcom`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END ) AS `over`, sum( CASE WHEN fare.diff_vat > 0 AND fare.ticket_type != 'A' AND fare.ticket_rounting != 'C' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `litter`, sum( CASE WHEN ( inv.inv_no IS NOT NULL AND inv.`status` != 2 ) THEN `fare`.`inv_amount` ELSE 0 END + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END + CASE WHEN ( fare.diff_vat < 0 OR fare.pv_type IS NOT NULL ) AND fare.ticket_type = 'TD' THEN ifnull(`fare`.agent_commission, 0) ELSE 0 END - CASE WHEN fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.inv_amount, 0) ELSE 0 END - CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.ticket_type = 'A' AND `fare`.`is_wait_pay` = 0 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat > 0 AND fare.ticket_type != 'A' AND fare.ticket_rounting != 'C' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR inv.inv_no = fare.remark ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`)) LEFT JOIN ticket_fare_airline fave ON fave.pv_code IS NOT NULL AND fare.id = fave.id ) " ; 
            querySum = " SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) WHEN fare.pv_code IS NOT NULL THEN substr(fare.pv_code, 1, 6) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' AND inv.inv_no IS NOT NULL ) THEN `fare`.`inv_amount` ELSE 0 END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' AND inv.inv_no IS NOT NULL ) THEN `fare`.`inv_amount` ELSE 0 END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' AND inv.inv_no IS NOT NULL ) THEN `fare`.`inv_amount` ELSE 0 END )), 0 ) AS `invoiceoutbound`, sum( CASE WHEN ( fare.diff_vat < 0 OR fare.pv_type IS NOT NULL ) AND fare.ticket_type = 'TD' THEN ifnull(`fare`.agent_commission, 0) ELSE 0 END ) AS `discount`, sum( CASE WHEN fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.inv_amount, 0) ELSE 0 END ) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.inv_amount, 0) ELSE 0 END )) AS `wait_pay`, sum( CASE WHEN fare.ticket_type = 'A' AND `fare`.`is_wait_pay` = 0 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `rcagcom`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END ) AS `over`, sum( CASE WHEN fare.diff_vat > 0 AND fare.ticket_type != 'A' AND fare.ticket_rounting != 'C' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `litter`, sum( CASE WHEN ( inv.inv_no IS NOT NULL AND inv.`status` != 2 ) THEN `fare`.`inv_amount` ELSE 0 END + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END + CASE WHEN ( fare.diff_vat < 0 OR fare.pv_type IS NOT NULL ) AND fare.ticket_type = 'TD' THEN ifnull(`fare`.agent_commission, 0) ELSE 0 END - CASE WHEN fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.inv_amount, 0) ELSE 0 END - CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.ticket_type = 'A' AND `fare`.`is_wait_pay` = 0 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat > 0 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR inv.inv_no = fare.remark ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`)) LEFT JOIN ticket_fare_airline fave ON fave.pv_code IS NOT NULL AND fare.id = fave.id )  ";
        }else{
...
//            sumticket.setTicketissueSum(util.ConvertString(B[3])== null ? "" : util.ConvertString(B[3]));
    ...
    public List<ListTicketCommissionReceive> getTicketCommissionReceive(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            ...
            query = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, sum( ifnull( `fare`.`ticket_commission`, 0 )) AS `comairline`, sum( ifnull( `fare`.`litter_commission`, 0 )) AS `littlecom`, sum( ifnull( `fare`.`agent_commission`, 0 )) AS `payagent`, sum( ifnull(`refd`.`agent_comission`, 0)) AS `rcagent`, sum( ifnull( `refd`.`airline_comission`, 0 )) AS `payrefund`, sum((((( ifnull( `fare`.`ticket_commission`, 0 ) + ifnull( `fare`.`litter_commission`, 0 )) - ifnull( `fare`.`agent_commission`, 0 )) + ifnull(`refd`.`agent_comission`, 0)) - ifnull( `refd`.`airline_comission`, 0 ))) AS `comreceive` FROM ((((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR fare.remark = inv.inv_no ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` )))) LEFT JOIN `refund_airticket_detail` `refd` ON (( `refd`.`ticket_no` = `fare`.ticket_no )) LEFT JOIN ticket_fare_airline fave ON fave.pv_code IS NOT NULL AND fare.id = fave.id ) ";
            //querySum = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, sum( ifnull( `fare`.`ticket_commission`, 0 )) AS `comairline`, sum( ifnull( `fare`.`litter_commission`, 0 )) AS `littlecom`, sum( ifnull( `fare`.`agent_commission`, 0 )) AS `payagent`, sum( ifnull(`refd`.`agent_comission`, 0)) AS `rcagent`, sum( ifnull( `refd`.`airline_comission`, 0 )) AS `payrefund`, sum((((( ifnull( `fare`.`ticket_commission`, 0 ) + ifnull( `fare`.`litter_commission`, 0 )) - ifnull( `fare`.`agent_commission`, 0 )) + ifnull(`refd`.`agent_comission`, 0)) - ifnull( `refd`.`airline_comission`, 0 ))) AS `comreceive` FROM ((((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` )))) LEFT JOIN `refund_airticket_detail` `refd` ON (( `refd`.`ticket_no` = `fare`.ticket_no ))) ";
            querySum = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air` , sum( ifnull( `fare`.`ticket_commission`, 0 )) AS `comairline`, sum( ifnull( `fare`.`litter_commission`, 0 )) AS `littlecom`, sum( ifnull( `fare`.`agent_commission`, 0 )) AS `payagent`, sum( ifnull(`refd`.`agent_comission`, 0)) AS `rcagent`, sum( ifnull( `refd`.`airline_comission`, 0 )) AS `payrefund`, sum((((( ifnull( `fare`.`ticket_commission`, 0 ) + ifnull( `fare`.`litter_commission`, 0 )) - ifnull( `fare`.`agent_commission`, 0 )) + ifnull(`refd`.`agent_comission`, 0)) - ifnull( `refd`.`airline_comission`, 0 ))) AS `comreceive` FROM ((((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` )))) LEFT JOIN `refund_airticket_detail` `refd` ON (( `refd`.`ticket_no` = `fare`.ticket_no )))";
        }else{  
            query = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, sum( ifnull( `fare`.`ticket_commission`, 0 )) AS `comairline`, sum( ifnull( `fare`.`litter_commission`, 0 )) AS `littlecom`, sum( ifnull( `fare`.`agent_commission`, 0 )) AS `payagent`, sum( ifnull(`refd`.`agent_comission`, 0)) AS `rcagent`, sum( ifnull( `refd`.`airline_comission`, 0 )) AS `payrefund`, sum((((( ifnull( `fare`.`ticket_commission`, 0 ) + ifnull( `fare`.`litter_commission`, 0 )) - ifnull( `fare`.`agent_commission`, 0 )) + ifnull(`refd`.`agent_comission`, 0)) - ifnull( `refd`.`airline_comission`, 0 ))) AS `comreceive` FROM ((((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR fare.remark = inv.inv_no ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` )))) LEFT JOIN `refund_airticket_detail` `refd` ON (( `refd`.`ticket_no` = `fare`.ticket_no )) LEFT JOIN ticket_fare_airline fave ON fave.pv_code IS NOT NULL AND fare.id = fave.id ) where " ;
