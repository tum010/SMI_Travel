...
public class SummaryTicketAdjustCostAndIncomeImpl implements SummaryTicketAdjustCostAndIncomeDao{
    ...
    public List getSummaryTicketCostAndIncome(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        System.out.println("Term Pay : " + termPayt);
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            query = "select (case when (`fare`.`ticket_type` = 'B') then 'BSP' when (`fare`.`ticket_type` = 'A') then 'AGENT' when (`fare`.`ticket_type` = 'D') then 'DOMESTIC' when (`fare`.`ticket_type` = 'TI') then 'TG INTER' when (`fare`.`ticket_type` = 'TD') then 'TG DOMESTIC' else `fare`.`ticket_type` end) AS `typepayment`,`fare`.`ticket_rounting` AS `typerounting`,count(`fare`.`ticket_no`) AS `pax`,substr(`fare`.`ticket_no`,1,3) AS `air`,`GET_TICKET_ISSUE`(`fare`.`ticket_type`,`fare`.`ticket_rounting`,substr(`fare`.`ticket_no`,1,3)) AS `ticketissue`,(case when (`inv`.`inv_type` = 'T') then substr(`inv`.`inv_no`,1,5) else substr(`inv`.`inv_no`,1,6) end) AS `invno`,sum(`GET_COST_TICKETFARE`(`fare`.`id`)) AS `cost`,sum((case when ((`fare`.`department` = 'inbound') and isnull(`fare`.`pv_type`)) then `fare`.`sale_price` end)) AS `inbound`,sum((case when ((`fare`.`department` = 'wendy') and isnull(`fare`.`pv_type`)) then `fare`.`sale_price` end)) AS `wendy`,sum((case when ((`fare`.`department` = 'outbound') and isnull(`fare`.`pv_type`)) then `fare`.`sale_price` end)) AS `outbound`,sum((case when (`fare`.`pv_type` = 9) then `fare`.`sale_price` else NULL end)) AS `refund`,sum((case when (`fare`.`pv_type` = 7) then `fare`.`sale_price` else NULL end)) AS `businesstrip`,sum((case when (`fare`.`pv_type` = 8) then `fare`.`sale_price` else NULL end)) AS `annualleave`,sum((case when (`fare`.`pv_type` = 10) then `fare`.`sale_price` else NULL end)) AS `noinvoice`,sum((`GET_COST_TICKETFARE`(`fare`.`id`) - `fare`.`sale_price`)) AS `costinv`,sum((case when ((`fare`.`department` = 'Wendy') and isnull(`fare`.`pv_type`)) then `fare`.`inv_amount` else NULL end)) AS `invwendy`,sum((case when ((`fare`.`department` = 'Outbound') and isnull(`fare`.`pv_type`)) then `fare`.`inv_amount` else NULL end)) AS `invoutbound`,sum((case when (`fare`.`pv_type` = 9) then `fare`.`inv_amount` else NULL end)) AS `invrefund`,sum((case when (`fare`.`pv_type` = 7) then `fare`.`inv_amount` else NULL end)) AS `invbusinesstrip`,sum((case when (`fare`.`pv_type` = 8) then `fare`.`inv_amount` else NULL end)) AS `invannualleave`,sum((case when (`fare`.`pv_type` = 10) then `fare`.`inv_amount` else NULL end)) AS `invnoinvoice` from (((`ticket_fare_airline` `fare` left join `ticket_fare_invoice` `finv` on((`finv`.`ticket_fare_id` = `fare`.`id`))) left join `invoice` `inv` on((`inv`.`id` = `finv`.`invoice_id`))) left join `m_accterm` `term` on((`term`.`id` = `inv`.`term_pay`))) " ; 
