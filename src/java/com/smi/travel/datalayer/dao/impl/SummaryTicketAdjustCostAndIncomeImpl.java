...
public class SummaryTicketAdjustCostAndIncomeImpl implements SummaryTicketAdjustCostAndIncomeDao{
    ...
    public List<ListSummaryTicketAdjustCostAndIncome> getSummaryTicketAdjustCostAndIncome(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            query = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceoutbound`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `discount`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END )) AS `wait_pay`, sum( CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `rcagcom`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END ) AS `over`, sum( CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `litter`, sum( `fare`.`inv_amount` + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR inv.inv_no = fare.remark ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`)))    " ; 
            querySum = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceoutbound`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `discount`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END )) AS `wait_pay`, sum( CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `rcagcom`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END ) AS `over`, sum( CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `litter`, sum( `fare`.`inv_amount` + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR inv.inv_no = inv.remark ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`)))   ";
        }else{
            query = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceoutbound`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `discount`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END )) AS `wait_pay`, sum( CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `rcagcom`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END ) AS `over`, sum( CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `litter`, sum( `fare`.`inv_amount` + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR inv.inv_no = fare.remark ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`)))  where " ;
            querySum = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceoutbound`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `discount`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END )) AS `wait_pay`, sum( CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `rcagcom`, sum( CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END ) AS `over`, sum( CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `litter`, sum( `fare`.`inv_amount` + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type != 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat *- 1, 0) ELSE 0 END + CASE WHEN fare.diff_vat < 0 AND fare.ticket_type = 'TD' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat < 0 AND fare.ticket_rounting = 'C' AND `fare`.`is_wait_pay` != 1 THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN (`fare`.`is_wait_pay` = 1) THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.ticket_type = 'A' AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END - CASE WHEN fare.diff_vat > 0 AND fare.ticket_rounting != 'C' AND `fare`.`is_wait_pay` != 1 AND fare.ticket_type != 'A' THEN ifnull(`fare`.diff_vat, 0) ELSE 0 END ) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` OR inv.inv_no = inv.remark ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`)))  where ";
        }
    ...
    public List getSummaryTicketCostAndIncome(String reportType, String invoiceFromDate, String invoiceToDate, String issueFrom, String issueTo, String paymentType, String departmentt, String salebyUser, String termPayt,String printby) {
        ...
        System.out.println("Term Pay : " + termPayt);
        if( invoiceFromDate == null  &&  invoiceToDate == null  && issueFrom == null && issueTo == null && paymentType == null && departmentt == null && salebyUser == null && termPayt == null){
            query = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, substr(`fare`.`ticket_no`, 1, 3) AS `air`, `GET_TICKET_ISSUE` ( `fare`.`ticket_type`, `fare`.`ticket_rounting`, substr(`fare`.`ticket_no`, 1, 3)) AS `ticketissue`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum( `GET_COST_TICKETFARE` (`fare`.`id`)) AS `cost`, sum(( CASE WHEN (( `fare`.`department` = 'inbound' ) AND isnull(`fare`.`pv_type`)) THEN `fare`.`sale_price` END )) AS `inbound`, sum(( CASE WHEN (( `fare`.`department` = 'wendy' AND (`fare`.`pv_type` = 10)) AND isnull(`fare`.`pv_type`)) THEN `fare`.`sale_price` END )) AS `wendy`, sum(( CASE WHEN (( `fare`.`department` = 'outbound' AND (`fare`.`pv_type` = 10)) AND isnull(`fare`.`pv_type`)) THEN `fare`.`sale_price` END )) AS `outbound`, sum(( CASE WHEN (`fare`.`pv_type` = 9) THEN `fare`.`sale_price` ELSE NULL END )) AS `refund`, sum(( CASE WHEN (`fare`.`pv_type` = 7) THEN `fare`.`sale_price` ELSE NULL END )) AS `businesstrip`, sum(( CASE WHEN (`fare`.`pv_type` = 8) THEN `fare`.`sale_price` ELSE NULL END )) AS `annualleave`, sum(( CASE WHEN (`fare`.`pv_type` = 10) THEN `fare`.`sale_price` ELSE NULL END )) AS `noinvoice`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - ( CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END ))) AS `costinv`, sum(( CASE WHEN (( `fare`.`department` = 'Wendy' ) AND isnull(`fare`.`pv_type`)) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invwendy`, sum(( CASE WHEN (( `fare`.`department` = 'Outbound' ) AND isnull(`fare`.`pv_type`)) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invoutbound`, sum(( CASE WHEN (`fare`.`pv_type` = 9) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invrefund`, sum(( CASE WHEN (`fare`.`pv_type` = 7) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invbusinesstrip`, sum(( CASE WHEN (`fare`.`pv_type` = 8) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invannualleave`, sum(( CASE WHEN (`fare`.`pv_type` = 10) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invnoinvoice` FROM ((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) " ; 
            querySum = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - ( CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END ))) AS `costinv`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'wendy' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoicewendy`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'inbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceinbound`, ifnull( sum(( CASE WHEN ( `fare`.`department` = 'outbound' ) THEN `fare`.`inv_amount` END )), 0 ) AS `invoiceoutbound`, sum( ifnull( `fare`.`agent_commission`, 0 )) AS `discount`, `GET_CANCEL_AMOUNT_TICKET_FARE` (`fare`.`id`) AS `cancel`, sum(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN `fare`.`inv_amount` ELSE 0 END )) AS `wait_pay`, sum( ifnull(`fare`.`diff_vat`, 0)) AS `rcagcom`, sum( ifnull(`fare`.`over_commission`, 0)) AS `over`, sum( ifnull( `fare`.`litter_commission`, 0 )) AS `litter`, sum((((((( ifnull(`fare`.`inv_amount`, 0) + ifnull(`fare`.`over_commission`, 0)) - ifnull( `fare`.`litter_commission`, 0 )) + ifnull( `fare`.`agent_commission`, 0 )) - ifnull(`fare`.`diff_vat`, 0)) - ifnull(( CASE WHEN (`fare`.`is_wait_pay` = 1) THEN `fare`.`inv_amount` ELSE 0 END ), 0 )) - ifnull( `GET_CANCEL_AMOUNT_TICKET_FARE` (`fare`.`id`), 0 ))) AS `total_balance` FROM (((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `staff` `st` ON ((`st`.`name` = `fare`.`owner`))) ";
        }else{
            ...
            querySum = "SELECT ( CASE WHEN (`fare`.`ticket_type` = 'B') THEN 'BSP' WHEN (`fare`.`ticket_type` = 'A') THEN 'AGENT' WHEN (`fare`.`ticket_type` = 'D') THEN 'DOMESTIC' WHEN (`fare`.`ticket_type` = 'TI') THEN 'TG INTER' WHEN (`fare`.`ticket_type` = 'TD') THEN 'TG DOMESTIC' ELSE `fare`.`ticket_type` END ) AS `typepayment`, `fare`.`ticket_rounting` AS `typerounting`, count(`fare`.`ticket_no`) AS `pax`, `GET_TICKET_ISSUE_SUMMARY` ( `fare`.`ticket_type`, `fare`.`ticket_rounting` ) AS `ticketissue`, ( CASE WHEN (`inv`.`inv_type` = 'T') THEN substr(`inv`.`inv_no`, 1, 5) ELSE substr(`inv`.`inv_no`, 1, 6) END ) AS `invno`, sum( `GET_COST_TICKETFARE` (`fare`.`id`)) AS `cost`, sum(( CASE WHEN (( `fare`.`department` = 'inbound' AND (`fare`.`pv_type` = 10)) AND isnull(`fare`.`pv_type`)) THEN `fare`.`sale_price` END )) AS `inbound`, sum(( CASE WHEN (( `fare`.`department` = 'wendy' AND (`fare`.`pv_type` = 10)) AND isnull(`fare`.`pv_type`)) THEN `fare`.`sale_price` END )) AS `wendy`, sum(( CASE WHEN (( `fare`.`department` = 'outbound' AND (`fare`.`pv_type` = 10)) AND isnull(`fare`.`pv_type`)) THEN `fare`.`sale_price` END )) AS `outbound`, sum(( CASE WHEN (`fare`.`pv_type` = 9) THEN `fare`.`sale_price` ELSE NULL END )) AS `refund`, sum(( CASE WHEN (`fare`.`pv_type` = 7) THEN `fare`.`sale_price` ELSE NULL END )) AS `businesstrip`, sum(( CASE WHEN (`fare`.`pv_type` = 8) THEN `fare`.`sale_price` ELSE NULL END )) AS `annualleave`, sum(( CASE WHEN (`fare`.`pv_type` = 10) THEN `fare`.`sale_price` ELSE NULL END )) AS `noinvoice`, sum(( `GET_COST_TICKETFARE` (`fare`.`id`) - CASE WHEN (`fare`.`pv_type` IS NOT NULL) THEN `fare`.`sale_price` ELSE 0 END )) AS `costinv`, sum(( CASE WHEN (( `fare`.`department` = 'Wendy' ) AND isnull(`fare`.`pv_type`)) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invwendy`, sum(( CASE WHEN (( `fare`.`department` = 'Outbound' ) AND isnull(`fare`.`pv_type`)) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invoutbound`, sum(( CASE WHEN (`fare`.`pv_type` = 9) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invrefund`, sum(( CASE WHEN (`fare`.`pv_type` = 7) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invbusinesstrip`, sum(( CASE WHEN (`fare`.`pv_type` = 8) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invannualleave`, sum(( CASE WHEN (`fare`.`pv_type` = 10) THEN `fare`.`inv_amount` ELSE NULL END )) AS `invnoinvoice` FROM ((( `ticket_fare_airline` `fare` LEFT JOIN `ticket_fare_invoice` `finv` ON (( `finv`.`ticket_fare_id` = `fare`.`id` ))) LEFT JOIN `invoice` `inv` ON (( `inv`.`id` = `finv`.`invoice_id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` )))   where ";
        }
