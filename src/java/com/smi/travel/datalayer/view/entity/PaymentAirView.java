/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.smi.travel.datalayer.view.entity;

/**
 *
 * @author Kanokporn
 */
public class PaymentAirView {
    private String payno;
    private String totalpay;
    private String totalcom;
    private String cash;
    private String chqno;
    private String amount;
    private String refund;
    private String comrefund;
    private String withtax;
    private String creditnote;
    private String debitnote;

    /**
     * @return the payno
     */
    public String getPayno() {
        return payno;
    }

    /**
     * @param payno the payno to set
     */
    public void setPayno(String payno) {
        this.payno = payno;
    }

    /**
     * @return the totalpay
     */
    public String getTotalpay() {
        return totalpay;
    }

    /**
     * @param totalpay the totalpay to set
     */
    public void setTotalpay(String totalpay) {
        this.totalpay = totalpay;
    }

    /**
     * @return the totalcom
     */
    public String getTotalcom() {
        return totalcom;
    }

    /**
     * @param totalcom the totalcom to set
     */
    public void setTotalcom(String totalcom) {
        this.totalcom = totalcom;
    }

    /**
     * @return the cash
     */
    public String getCash() {
        return cash;
    }

    /**
     * @param cash the cash to set
     */
    public void setCash(String cash) {
        this.cash = cash;
    }

    /**
     * @return the chqno
     */
    public String getChqno() {
        return chqno;
    }

    /**
     * @param chqno the chqno to set
     */
    public void setChqno(String chqno) {
        this.chqno = chqno;
    }

    /**
     * @return the amount
     */
    public String getAmount() {
        return amount;
    }

    /**
     * @param amount the amount to set
     */
    public void setAmount(String amount) {
        this.amount = amount;
    }

    /**
     * @return the refund
     */
    public String getRefund() {
        return refund;
    }

    /**
     * @param refund the refund to set
     */
    public void setRefund(String refund) {
        this.refund = refund;
    }

    /**
     * @return the comrefund
     */
    public String getComrefund() {
        return comrefund;
    }

    /**
     * @param comrefund the comrefund to set
     */
    public void setComrefund(String comrefund) {
        this.comrefund = comrefund;
    }

    /**
     * @return the withtax
     */
    public String getWithtax() {
        return withtax;
    }

    /**
     * @param withtax the withtax to set
     */
    public void setWithtax(String withtax) {
        this.withtax = withtax;
    }

    /**
     * @return the creditnote
     */
    public String getCreditnote() {
        return creditnote;
    }

    /**
     * @param creditnote the creditnote to set
     */
    public void setCreditnote(String creditnote) {
        this.creditnote = creditnote;
    }

    /**
     * @return the debitnote
     */
    public String getDebitnote() {
        return debitnote;
    }

    /**
     * @param debitnote the debitnote to set
     */
    public void setDebitnote(String debitnote) {
        this.debitnote = debitnote;
    }
    
}
