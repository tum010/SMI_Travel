...
//    
    ...
    public List getInvoiceSummary(String fromData, String toDate, String department, String type,String agent,String statusInvoice,String printBy,String subdepartment) {
        ...
        if("".equals(department)  && "".equals(type)  && "".equals(fromData)  && "".equals(toDate) && "".equals(agent)){
            query = "SELECT `inv`.`inv_no` AS `invno`, `inv`.`inv_date` AS `invdate`, `inv`.`inv_to` AS `to`, `inv`.`inv_name` AS `invname`, `term`.`name` AS `termpay`, group_concat( DISTINCT `bt`.`name` SEPARATOR ',' ) AS `detail`, round( sum(( CASE WHEN (`invd`.`is_vat` = 0) OR inv.inv_type = 'T' THEN `invd`.`amount_local` ELSE ((`invd`.`amount_local` * 100) / (100 + `invd`.`vat`)) END )), 2 ) AS `gross`, round( sum(( CASE WHEN (`invd`.`is_vat` = 0) OR inv.inv_type = 'T' THEN 0 ELSE ( `invd`.`amount_local` - ((`invd`.`amount_local` * 100) / (100 + `invd`.`vat`))) END )), 2 ) AS `vat`, round(sum(`invd`.`amount`), 2) AS `amount`, round(sum(`invd`.`cost_local`), 2) AS `cost`, round( sum(( `invd`.`amount_local` - `invd`.`cost_local` )), 2 ) AS `profit`, round( sum(`invd`.`amount_local`), 2 ) AS `amountlocal`, `invd`.`cur_amount` AS `cur`, `st`.`username` AS `staff`, ( CASE WHEN (`fi`.`name` = 'NORMAL') THEN 'NORMAL' WHEN (`fi`.`name` = 'VOID') THEN 'VOID' ELSE 'VOID' END ) AS `status`, `inv`.`department` AS `department`, `inv`.`inv_type` AS `type`, `inv`.`sub_department` AS `subdepartment` FROM ((((( `invoice` `inv` JOIN `invoice_detail` `invd` ON (( `invd`.`invoice_id` = `inv`.`id` ))) LEFT JOIN `m_accterm` `term` ON (( `term`.`id` = `inv`.`term_pay` ))) LEFT JOIN `m_billtype` `bt` ON (( `bt`.`id` = `invd`.`item_type_id` ))) LEFT JOIN `staff` `st` ON ((`st`.`id` = `inv`.`staff_id`))) LEFT JOIN `m_finance_itemstatus` `fi` ON ((`fi`.`id` = `inv`.`status`))) " ; 
        }else{
