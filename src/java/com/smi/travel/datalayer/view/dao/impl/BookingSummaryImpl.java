...
//        String query = "SELECT * FROM `booking_header_summary` where  refno = '"+refno+"'";
    ...
    public List getOutboundStaffSummaryList(String from, String to, String saleby, String currency) {
        ...
        String query = "SELECT st.`name` AS staff, pt. NAME AS productType, sum( CASE WHEN (bd.cur_cost <> 'THB') THEN ((bd.cost) * ifnull(bd.ex_rate, 1)) ELSE bd.cost END ) AS cost, sum(bd.price) AS price, sum( bd.price - ( CASE WHEN (bd.cur_cost <> 'THB') THEN ((bd.cost) * ifnull(bd.ex_rate, 1)) ELSE bd.cost END )) AS profit FROM `billable` bi INNER JOIN billable_desc bd ON bi.id = bd.billable_id INNER JOIN `master` mt ON mt.id = bi.master_id INNER JOIN m_billtype pt ON pt.id = bd.bill_type INNER JOIN staff st ON st.id = mt.Staff_id WHERE mt.booking_type = 'O' AND bd.bill_type <> 6 AND mt.Create_date BETWEEN '"+from+"' AND '"+to+"'";
