...
public class SaleVatReportImpl implements SaleVatReportDao{
    ...
    public List getSaleVatReportList(String month, String year, String department) {
        ...
        String query = "SELECT * FROM `output_tax_view` where month(taxdate) = '"+month+"' and YEAR(taxdate) = '"+year+"' and (status = 'Post' || status = 'Change' || status = 'Void' || status = 'VoidPost') ";

        if ((department != null) && (!"".equalsIgnoreCase(department))) {
            query += " and department = '" + department + "'" ;
        }
        
        query += " ORDER BY taxdate , taxno , tax_type ";
